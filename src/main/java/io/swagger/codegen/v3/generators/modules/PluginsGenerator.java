...
public class PluginsGenerator extends AbstractGenerator implements Generator {
    ...
    //custom property
    //checking can genrate service
    protected Boolean generateService = null;
    //checking can generate entities
    protected Boolean generateEntities = null;
    //checking can generate vmodel
    protected Boolean generateVmodel = null;
    ...
    protected void configureGeneratorProperties() {
        ...
        if (generateApis == null && generateModels == null && generateSupportingFiles == null && generateVmodel == null) {
            // no specifics are set, generate everything
            generateEntities = generateVmodel = generateApis = generateModels = generateSupportingFiles = true;
        } else {
            ...
            if (generateVmodel == null) {
                generateVmodel = false;
            }
    ...
    protected void generateModels(List<File> files, List<Object> allModels) {
        ...
        //get components from all components objecct and schemas object
        final Map<String, Schema> schemas = this.openAPI.getComponents().getSchemas();
        //check if schema null will not proceed
        if (schemas == null) {
            return;
        }
        ...
        Map<String, Object> allProcessedModels = new TreeMap<>(new Comparator<String>() {
            ...
            public int compare(String o1, String o2) {
                return ObjectUtils.compare(plugins.toModelName(o1), plugins.toModelName(o2));
            }
        });
        ...
        for (String name : modelKeys) {
            try {
                ...
                Map<String, Object> models = processModels(plugins, schemaMap, schemas);
                models.put("classname", plugins.toModelName(name));
                ...
                for (Object object : modelList) {
                    ...
                    if (codegenModel != null) {
                        models = processModel(codegenModel, plugins, schemas);
                        models.put("classname", plugins.toModelName(codegenModel.name));
                        models.putAll(plugins.additionalProperties());
        ...
        for (String modelName : allProcessedModels.keySet()) {
            ...
            try {
                ...
                for (String templateName : plugins.modelTemplateFiles().keySet()) {
                    String suffix = plugins.modelTemplateFiles().get(templateName);
                    String filename = plugins.modelFileFolder() + File.separator + plugins.toModelFilename(modelName) + suffix;
    ...
    public List<File> generate() {
        ...
        if (this.pluginExtensions.length != 0) {
    ...
    private void generateEntitiesFile(List<File> files, List<Object> allEntities) {
        ...
        for (String modelName : allProcessedModels.keySet()) {
            ...
            try {
                ...
                Map<String, Object> modelTemplate = (Map<String, Object>) ((List<Object>) models.get("models")).get(0);
                if (isJavaCodegen(plugins.getName())) {
                    // Special handling of aliases only applies to Java
                    if (modelTemplate != null && modelTemplate.containsKey("model")) {
                        CodegenModel codegenModel = (CodegenModel) modelTemplate.get("model");
                        Map<String, Object> vendorExtensions = codegenModel.getVendorExtensions();
    ...
    protected void generateVmodel(List<File> files, List<Object> allModels) {
        ...
        for (String modelName : allProcessedModels.keySet()) {
            ...
            try {
                ...
                if (isJavaCodegen(plugins.getName())) {
                    // Special handling of aliases only applies to Java
                    if (vModels != null && vModels.containsKey("model")) {
                        CodegenModel codegenModel = (CodegenModel) vModels.get("model");
                        Map<String, Object> vendorExtensions = codegenModel.getVendorExtensions();
                        boolean isAlias = false;
                        if (vendorExtensions.get(CodegenConstants.IS_ALIAS_EXT_NAME) != null) {
                            isAlias = Boolean.parseBoolean(vendorExtensions.get(CodegenConstants.IS_ALIAS_EXT_NAME).toString());
...
//                }
    ...
    protected Map<String, Object> processEntitiesies(Plugins config, Map<String, EntitiesSchemas> definitions, Map<String, EntitiesSchemas> allDefinitions) {
        Map<String, Object> objs = new HashMap<>();
        ...
        for (String key : definitions.keySet()) {
            EntitiesSchemas schema = definitions.get(key);
            CodegenModel cm = plugins.fromEntities(key, schema, allDefinitions);
            Map<String, Object> mo = new HashMap<>();
            mo.put("model", cm);
            mo.put("importPath", plugins.toModelImport(cm.classname));
            mo.put("vmodels", plugins.modelPackage() + cm.classname);
    ...
    protected Map<String, Object> processModels(CodegenConfig config, Map<String, Schema> definitions, Map<String, Schema> allDefinitions) {
        Map<String, Object> objs = new HashMap<>();
...
//            mo.put("vmodels",config.modelPackage()+ cm.classname);
    ...
    protected Map<String, Object> processModel(CodegenModel codegenModel, CodegenConfig config, Map<String, Schema> allDefinitions) {
        Map<String, Object> objs = new HashMap<>();
        objs.put("package", config.modelPackage());
        List<Object> models = new ArrayList<>();
    ...
    protected Map<String, SecurityScheme> getAuthMethods(List<SecurityRequirement> securities, Map<String, SecurityScheme> securitySchemes) {
    ...
    protected boolean isJavaCodegen(String name) {
        return name.equalsIgnoreCase("java")
            || name.equalsIgnoreCase("inflector");
    }

    protected Boolean getCustomOptionBooleanValue(String option) {
